...
class PercentileRewardScaler:
    """Normalizes rewards to a fixed range using running percentiles."""
...
class ParabolicReversalEnv(gym.Env):
    ...
    def __init__(
        ...
        self.reward_scaler = PercentileRewardScaler()
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict]:
        ...
        #   action < -0.15: INCREASE short exposure (Entry/Add)
        #   action > 0.05:  DECREASE short exposure (Cover)
        #   else:           HOLD current exposure  [-0.15, 0.05]
...
# === CIRCUIT BREAKER CHECK ===
        ...
        if action_mask[action_type] == 0:
            ...
            if self._mask_violation_count <= 3:
                logger.warning(
                    f"MASK VIOLATION #{self._mask_violation_count}: Action type {action_type} blocked by mask. "
        ...
        # FIX 1: reward_scale applied AFTER percentile scaler
        reward = self.reward_scaler.scale(raw_reward) * self.config.reward_scale

        # Masking penalty applied after scaling (hard penalty, not shaping)
        if masking_violation:
            reward += self.config.masking_penalty
    ...
    def _compute_waiting_quality(self) -> float:
        """Reward strategic patience when flat."""
        if self.current_position != 0:
            return 0.0
        if not self.in_entry_window:
            return 0.02  # Being flat outside entry window
        if self.vwap_deviation < self.config.min_vwap_deviation_entry:
            return 0.01  # Below entry threshold — wait for setup
        return 0.0
    ...
    def _compute_trade_completion_bonus(
        ...
        """One-time bonus on position close. FIX 2: Capped at 1.5 total."""
    ...
    def _discretize_action(self, exposure_fraction: float) -> int:
        ...
        Returns:
            0: INCREASE short exposure (Entry) - exposure_fraction < -0.15
            1: DECREASE short exposure (Cover) - exposure_fraction > 0.05
            2: Hold (-0.15 <= exposure_fraction <= 0.05)
        """
