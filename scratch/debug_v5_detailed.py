#!/usr/bin/env python3
"""Detailed debug of V5 entry selection."""
import sys
sys.path.insert(0, '.')
...
print(f"Day open: ${day_open:.2f}, Day high: ${day_high:.2f}")
print()

# Process bars like V5 does
volume_history = []
best_setup = None
...
for i, bar in enumerate(bars):
    ...
    if criteria_met >= 2:
        print(f"{t.strftime('%H:%M')}: close=${bar['close']:.2f}, vwap_ext={vwap_ext:.2f}, vol_ratio={vol_ratio:.2f}, prox={prox:.2f}, CRITERIA MET ({criteria_met}/3)")
        if best_setup is None or vwap_ext > best_setup['vwap_ext']:
            ...
            print(f"  -> NEW BEST SETUP")

print()
if best_setup:
    bar = best_setup['bar']
    print(f"BEST SETUP: {bar['time_et'].strftime('%H:%M')} @ ${bar['close']:.2f}")
    print(f"  VWAP ext: {best_setup['vwap_ext']:.2f}")
    print(f"  Vol ratio: {best_setup['vol_ratio']:.2f}")
    print(f"  Proximity: {best_setup['prox']:.2f}")
else:
