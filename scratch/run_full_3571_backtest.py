"""
Complete Fresh Backtest - ALL 3,571 Symbols with ML Risk Engine

This script runs the full backtest on ALL symbols from extended_universe.py
"""
...
# Use ALL symbols from extended universe
ALL_SYMBOLS = EXTENDED_MICRO_CAP_SYMBOLS
print(f"Loaded {len(ALL_SYMBOLS)} symbols from extended universe")
...
class FullBacktestEngine:
    """Complete backtest engine using ALL 3,571 symbols."""
    
    def __init__(self, start_date: datetime, end_date: datetime):
        self.start_date = start_date
        self.end_date = end_date
        self.symbols = ALL_SYMBOLS  # Use ALL symbols
        
        print(f"\n[INIT] Loaded {len(self.symbols)} symbols for scanning")
        ...
        self.output_dir = Path('reports/full_3571_backtest')
    ...
    def run_complete_backtest(self):
        ...
        print("\n" + "="*80)
        print("COMPLETE FRESH BACKTEST - ALL 3,571 SYMBOLS")
        print("="*80)
        print(f"Period: {self.start_date.date()} to {self.end_date.date()}")
        print(f"Symbols: {len(self.symbols)}")
        print(f"Estimated Setups: ~3,500")
    ...
    def _check_setup(self, symbol: str, date: datetime) -> Optional[Dict]:
        ...
        try:
            ...
            # 30% gain threshold (relaxed scanner)
            if day_gain < 30:
                return None
    ...
    def _print_progress(self, current_idx: int):
        ...
        print(f"Current: {self.stats['current_symbol']}")
        print("-"*80)
        ...
        print(f"ML: {self.stats['ml_trades_taken']} taken, {self.stats['ml_trades_blocked']} blocked, ${self.stats['ml_pnl']:,.0f} P&L")
        print(f"{'='*80}\n")
    ...
    def _generate_report(self):
        ...
        print("="*80)
        print(f"\nSymbols Scanned: {report['symbols_scanned']}")
        print(f"Setups Found: {report['total_setups_found']}")
        print(f"\nV5 Relaxed: {report['v5_relaxed']['trades_taken']} trades, ${report['v5_relaxed']['total_pnl']:,.0f}")
        print(f"V5 Institutional: {report['v5_institutional']['trades_taken']} trades, {report['v5_institutional']['trades_blocked']} blocked, ${report['v5_institutional']['total_pnl']:,.0f}")
        print(f"\nWinner: {report['comparison']['recommendation']} (P&L diff: ${pnl_diff:+,.0f})")
        print("="*80)


def main():
    print("="*80)
    print("FULL 3,571 SYMBOL BACKTEST WITH ML RISK ENGINE")
    print("="*80)
    print(f"\nSymbols: {len(ALL_SYMBOLS)}")
    print(f"Period: 2019-01-01 to 2024-12-31")
    print(f"Estimated Runtime: 8-12 hours")
    print("\nPress Ctrl+C to cancel, or wait to start automatically...")
    time.sleep(5)
    
    start_date = datetime(2019, 1, 1)
    end_date = datetime(2024, 12, 31)
    
    engine = FullBacktestEngine(start_date, end_date)
