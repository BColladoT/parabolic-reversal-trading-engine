...
print()
print("Bars around 13:54 (index 380-400):")
for i, row in enumerate(bar_df.to_dicts()[380:400]):
    ts = row['timestamp'].astimezone(et_tz)
    print(f"{i+380}: {ts.strftime('%H:%M')}: Open={row['open']:.2f}, High={row['high']:.2f}, Close={row['close']:.2f}")
